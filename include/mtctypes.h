...
// MTC_UUID
//
//      Universally unique identifier. Also known as a GUID.
//
//--

#define MTC_UUID_SIZE   (32)
typedef MTC_S8          MTC_UUID[MTC_UUID_SIZE];
typedef MTC_UUID        *PMTC_UUID;

#define UUID_comp(X, Y) strncasecmp(X, Y, MTC_UUID_SIZE)
#define UUID_cpy(X, Y)  memcpy(X, Y, MTC_UUID_SIZE)
#define UUID_zero       "00000000000000000000000000000000"


//++
//
//      64 bit monotonic clock counter from node boot in mili-second
//
//--

typedef MTC_S64         MTC_CLOCK, *PMTC_CLOCK;

#define tstoms(X)       ((MTC_CLOCK) ((X).tv_sec) * 1000 + (X).tv_nsec / 1000 / 1000)
#define tvtoms(X)       ((MTC_CLOCK) ((X).tv_sec) * 1000 + (X).tv_usec / 1000)
...
MTC_ASSERT_SIZE(sizeof(MTC_UUID) == MTC_UUID_SIZE);
...
//
//  static keyword for function.
//  To export function name to log_backtrace,
//  All static functions are defined as global.
//
